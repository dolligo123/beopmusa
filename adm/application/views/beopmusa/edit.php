    <form name="form" action="/adm/beopmusa/post" enctype="multipart/form-data" method="post" onsubmit="return chkval();">
      ...
      <input type="hidden" id="bp_uid_chk" />
      ...
      <main class="main-wrap">
        <div class="card-wrap">
          ...
          <dl>
            ...
            <dd>
              <input type="text" name="bp_uid" placeholder="영문아이디를 입력하세요." maxlength="13" value="<?= $beopmusa['bp_uid'] ?>" />
            </dd>
          </dl>
          <dl>
            <dt>
          ...
          <dl>
            ...
            <dd>
              <ul class="add-list clearfx">
                <li>
                  <?php if ($beopmusa['intro_photo']) { ?>
                    <a href="<?= $beopmusa['intro_photo'] ?>" target="_blank">
                      <img src="<?= $beopmusa['intro_photo'] ?>" style="max-width:500px;" />
                    </a>
                  <?php } else { ?>
                    <a href="javascript:void(0);" target="_blank"></a>
                  <?php } ?>
                  <input type="file" name="intro_photo" id="intro_photo" />
                  <span class="file-btn" style="cursor: pointer;" onclick="$('#intro_photo').click()">파일선택</span>
                  <input type="checkbox" name="intro_photo_del" value="1" id="img-chk-0">
                  <label for="img-chk-0">사진 삭제</label>
                </li>
              </ul>
            </dd>
    ...
    <script type="text/javascript">
      ...
      // check bp_uid        
      function check_bp_uid() {
        if ($("input[name='bp_uid']").val()) {
          var query = {
            "bp_uid": $("input[name='bp_uid']").val(),
            "bp_id": $("input[name='bp_id']").val()
          };
          $.ajax({
            type: "post",
      ...
      $("input[name='bp_uid']").keyup(function() {
        check_bp_uid();
      });
    </script>