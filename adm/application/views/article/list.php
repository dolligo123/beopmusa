        ...
        <div class="card-wrap">
            ...
            <div class="top-btn-wrap">
                <ul class="clearfx">
                    ...
                    <li><a href="/adm/article/edit" class="type1-btn">등록하기</a></li>
		...
		$(document).ready(function() {
		        ...
		        "columns": [	        
					...
					{ 
			        	...
			        	"render" : function (val, type, row) {
		                    return "<a href='/adm/article/edit/" + row.at_id + "' class='cssview' >" + val + "</a>";
		            	}
			        },
					{ 
			            "orderable": false,
			        ...
			        { 
			        	...
			        	"render" : function (val, type, row) {
				        	var retval = "";
				        	if (val == 1) {
					        	retval = '<button class="openClose-btn" data-id="' + row.at_id + '">공개</button>';
				        	} else {
					        	retval = '<button class="openClose-btn" data-id="' + row.at_id + '">비공개</button>';
				        	}
				        	return retval += '<button class="del-btn" data-id="' + row.at_id + '">삭제</button>';
		                }
			        }
		        ]
		    });
			...
			$(document).on("click", ".openClose-btn", function(){	
                if (confirm("공개설정 변경 하겠습니까?")) {
                    ...
                    $.ajax({
                        type: "post",
                        url: "/adm/article/isopen/",
                        data: {'at_id' : obj.attr("data-id")},
			...
			$(document).on("click", ".del-btn", function(){	
				if (confirm("삭제하면 복구 안됩니다. 삭제 하시겠습니까?")) {
					...
					$.ajax({
					    type: "post",
					    url: "/adm/article/delete/" + obj.attr("data-id"),
					    data: {'at_id' : obj.attr("data-id")},
					    beforeSend : function(xhr, opts) {
					    },
					    success: function(result) {
			...
			$("#excel").click(function(e){
				...
				var url = "/adm/article/json?isexcel=true" + param;
                    ...
                    success: function (data) { 
                        var excelHandler = {
                            getExcelFileName : function(){
                                var filename = "미디어목록_" + "<?=date('YmdHis')?>" + ".xlsx";
                                return filename;
                            },
