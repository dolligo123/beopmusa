...
class Counselq extends Parents
{
  ...
  private function verify_recaptcha()
  {
    ...
    if (empty($token)) {
      log_message('error', 'reCAPTCHA [counselq]: token empty');
      return false;
    }
    ...
    if ($curl_error) {
      log_message('error', 'reCAPTCHA [counselq] cURL error: ' . $curl_error);
      return false;
    }

    $result = json_decode($response, true);
    log_message('debug', 'reCAPTCHA [counselq] result: ' . $response);
