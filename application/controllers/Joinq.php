...
class Joinq extends Parents {
	...
	private function sendsms($jq_id) {
        ...
        $hp_name = "<h3>법무사명 : " . $joinq['data'][0]['hp_name'] . "</h3>";
        $jq_name = "<h3>담당자명 : " . $joinq['data'][0]['jq_name'] . "</h3>";
        $tel = "<h3>연락처 : " . $joinq['data'][0]['tel'] . "</h3>";
        $email = "<h3>이메일 : " . $joinq['data'][0]['email'] . "</h3>";
        ...
        if ($r_phone) {
            $message = "<h2>[하나카N] ".$joinq['data'][0]['jq_name']."님의 문의가 도착했습니다.</h2>" . 
                        $hp_name . $jq_name . $tel . $email . $local . $jq_desc;
            
            $title = "하나카N " . $joinq['data'][0]['jq_name']."님의 문의가 도착했습니다.";
